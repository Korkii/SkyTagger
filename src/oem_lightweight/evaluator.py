...
class SegEvaluator(object):
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            transform = transforms.Compose(
                [transforms.ToTensor(), transforms.Normalize(**self.stats)])
    ...
    def show_predicted_image(self, pred, only_pred=False, fig_size=None, save_img=True, save_dir=None):
        ...
        if not only_pred:
            ...
            ax.text(0.16, -0.02, 'Image', horizontalalignment='center',
                    verticalalignment='top', transform=ax.transAxes)
            ax.text(0.5, -0.02, 'Segmentation', horizontalalignment='center',
                    verticalalignment='top', transform=ax.transAxes)
            ax.text(0.83, -0.02, 'Reference', horizontalalignment='center',
                    verticalalignment='top', transform=ax.transAxes)

        # create a patch for every color and use for legend
        patches = [mpatches.Patch(facecolor=np.array(
            self.class_colors[i]) / 255., label=self.class_names[i], linewidth=3) for i in range(8)]
        ...
        if save_img:
            if save_dir is not None:
                ...
                cv2.imwrite(os.path.join(
                    save_dir, "SparseMask"+os.path.basename(name).replace(" ", "_") + ".png"), comp_img)
            else:
                cv2.imwrite(os.path.join(os.path.realpath('.'),
                            "SparseMask"+name.replace(" ", "_") + ".png"), comp_img)
