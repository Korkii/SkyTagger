...
def _set_img_color(colors, background, img, gt, show255=False, weight_foreground=0.55):
    ...
    cv2.addWeighted(img, weight_foreground, origin,
                    (1 - weight_foreground), 0, img)
    return img
...
def prepare_data(img_file, label_file):
    ...
    data_dict = dict(img=img, label=label,
                     filename=str(item_name.capitalize()))
...
def _fetch_data(img_file, label_file):
    ...
    print(img.shape, "from fetch data")
    return img, label
